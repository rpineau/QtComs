...
int CRTIDome::Connect(const char *pszPort, const char *IpAddr, int nIpPort)
{
    ...
    if(!m_bNetworkConnected) {
        ...
        m_DeviceName.assign(IpAddr);
        m_nDevicePort = nIpPort;
        bConnectionOpen = m_devicePort->open(IpAddr, nIpPort);
        if(bConnectionOpen)
            m_bIsConnected = true;
        else {
            m_bIsConnected = true;
...
void CRTIDome::setConnectionType(const int nConnectionType)
{
    m_DeviceConnectionType = nConnectionType;
}

void CRTIDome::getConnectionType(int &nConnectionType)
{
